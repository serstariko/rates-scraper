...
- Чекбокс **«Загружать данные из Cbonds»** (по умолчанию выключен).
- В UI есть блок авторизации Cbonds (логин/пароль) для закрытых источников.
- Поддержан импорт файла Cbonds (XLSX/XLS/CSV): если `ID индекса` из файла совпадает с нужной ставкой, значения берутся из файла.
- В блоке результатов: основная таблица, технические детали и фильтр по статусу (`ok`, `no_rate_found`, `error`).
- На вкладке ставок выполняется валидация `Дата текущей` против требуемой даты (календарь + shift + выходные + праздники); при несовпадении ячейка подсвечивается красным.
- Все метки времени в интерфейсе отображаются в московском времени (МСК).
- Excel выгружается с именем `OTC_MD_dd mm yyyy.xlsx` (дата выгрузки в МСК).
- Отдельная вкладка **Календари** с редактируемой таблицей праздничных дней.
- Поддержаны календари: `Beijing`, `Europe`, `NewYork`, `RUONIA`, `RUSFAR`, `RUSFARCNY`, `MOSCOW`, `SPFI`.
- Даты в календарях можно добавлять/удалять в UI и сохранять в локальный файл `.holiday_calendars.json`.
- Отдельная вкладка **Маппинг ставок** со строками, синхронизированными со вкладкой ставок (`source_name`), и редактируемыми полями `Calendar` и `Shift`.
- Для поля `Calendar` используется выпадающий список доступных календарей.
- Маппинг сохраняется в локальный файл `.rate_calendar_mapping.json`.
